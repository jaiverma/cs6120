...
module BasicBlock = struct
    ...
    let make_bb ?label:(label=(sprintf "L%s" !_label)) instrs =
        if String.equal label !_label then
            _label := string_of_int (int_of_string (!_label) + 1)
        else
            ();
        { label=sprintf "%s" label; instrs=instrs }
...
module CFG = struct
    ...
    let make_cfg ~fn ~blocks =
        ...
        let rec build_cfg (bbs': BasicBlock.t list) =
            ...
            | bb::bbs ->
                ...
                let _ = match (last |> member "op" |> to_string) with
                | "jmp" ->
                    (* jmp is guaranteed to have only one target label *)
                    let target = match (last |> member "labels" |> to_list) with
                        | x::[] -> x |> to_string
                ...
                | "br" ->
                    ...
                    in
                    edges := (bb.label, target_a) ::
                        (bb.label, target_b) :: !edges
        ...
        { fn=fn; bbs=(to_assoc_list blocks); edges=(!edges) }
    ...
    let dot cfg =
        ...
        let nodes = List.map ~f:(fun (label, _) ->
            sprintf "\t%s;\n" label) cfg.bbs in
...
let () =
    let open Yojson.Basic.Util in
    let fn_map = Yojson.Basic.from_file "add.json"
    |> member "functions"
    |> to_list
    |> List.map ~f:(fun fn ->
    ...
    let cfg_map = List.map ~f:(fun (fn_name, bbs) ->
        CFG.make_cfg ~fn:fn_name ~blocks:bbs) fn_map
